...
describe("User login and create sale invoice", () => {
  ...
  it("should be created successfully", () => {
    ...
    cy.get(
      '.select-country-container > .country-selector > .country-selector__list > .vue-recycle-scroller > .vue-recycle-scroller__item-wrapper > [style="transform: translateY(0px);"] > .flex'
    ).type(phoneNumber);

    cy.get(":nth-child(2) > .input").click(); // Click to open the date picker
    cy.get(
      ".flatpickr-calendar.open > .flatpickr-innerContainer > .flatpickr-rContainer > .flatpickr-days > .dayContainer"
    )
      .contains(pickupDay)
      .scrollIntoView()
